                ...
                case SPAN:
                    grandChild.innerHTML =
                        `<div class="input-group-append">
                            <button class="btn btn-outline-danger" type="button" onclick="remove(${i})" title="Remove file #${i + 1} (${formatSpecificFileSize('file-' + (i+1))})">
...
function handleListView(handle = true) {
    if (handle === true) {
        ...
        if (len === 0) {
            ...
            let s = "";
            if (len > 1) s = "s";
            fileInfo.innerHTML = `The File${s} selected (${len} file${s} - ${totalFileSize()}):`;
...
function validate() {
    ...
    const allow = title && description && date && subs && auths;
    document.getElementById('submitButton').disabled = !allow;
    allowReload = allow;

    if (!title && !date && !auths && !subs && !description) {
        allowReload = true;
    }

    if (allow) {
        pHeading.innerHTML = "<b>Valid</b>";
        pMsg.innerHTML = "The form can be submitted";
        document.getElementById('close-btn-progress').style.display = 'block';
    } else {
        pHeading.innerHTML = "Invalid ";
        pMsg.innerHTML = "Please complete the following";
...
function genOrphanedListItem(id, name) {
    ...
    if (name.endsWith('.pdf')) {
        _eOFiles.files.push({
            id: id,
            url: `fetch.file.php?file=${encodedId}`,
            name: name
        });
    }
    ...
    return `<li class="list-group-item d-flex justify-content-between align-items-center" id="${id}-${name}">
        ...
        <div class="input-group-append">
            ...
            &nbsp;
            <button type="button" class="btn btn-outline-danger" title="Remove '${name}' from the list"
...
const inputEFiles = document.getElementById('e-files-selected');
...
function addExistingFile() {
    if (eFileElement.selectedIndex > -1) {
        ...
        const id = selected.value;
        const name = selected.innerText;
        eFileElement.remove(eFileElement.selectedIndex);

        if (eFileElement.options.length === 0) {
            addEFileBtn.disabled = true;
        }

        eFileList.innerHTML += genExistingListItem(id, name);

        if (inputEFiles.value === "") {
            inputEFiles.value = `${id}`;
        } else {
            inputEFiles.value += `, ${id}`;
        }
    }
...
function removeEFileFromList(id, name) {
    ...
    if (index > -1) {
        ...
        // remove from list
        let children = eFileList.children;
        eFileList.removeChild(children[index]);

        //add back to select
        eFileElement.innerHTML += `<option value="${id}">${name}</option>`;

        if (eFileElement.options.length > 0) {
            addEFileBtn.disabled = false;
        }
    }
}
...
function genExistingListItem(id, name) {
    ...
    return `<li class="list-group-item d-flex justify-content-between align-items-center" id="${id}-${name}">
        ...
        <div class="input-group-append">
            ...
            &nbsp;
            <button type="button" class="btn btn-outline-danger" title="Remove '${name}' from the list"
            onclick="removeEFileFromList('${id}', '${name}')">
                <i class="fas fa-unlink"></i>
            </button>
        </div>
    </li>`
