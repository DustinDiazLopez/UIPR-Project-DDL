...
define('SQL_GET_FILE', 'SELECT `id`, `path` FROM `file` WHERE `id` = ');
...
define('SQL_GET_ORPHANED_FILES', 'SELECT f.id, f.`path` FROM `file` f WHERE NOT EXISTS (SELECT * FROM file_has_item fi WHERE f.id = fi.item_id)');
